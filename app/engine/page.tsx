...
const step1Schema = z.object({
  ...
  if (!getAllowedSwapCities(data.desiredState).includes(data.desiredCity)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['desiredCity'], message: 'Select a valid city' })
  }
})
...
const step2Schema = z.object({
  ...
  .superRefine((data, ctx) => {
    if (!getAllowedSwapCities(data.currentState).includes(data.currentCity)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['currentCity'], message: 'Select a valid city' })
    }
    const areas = getSwapAreasForCity(data.currentState, data.currentCity)
    if (areas.length > 0) {
      if (!data.currentArea) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['currentArea'], message: 'Select an area' })
        return
      }
      if (!areas.includes(data.currentArea)) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['currentArea'], message: 'Select a valid area' })
      }
    }
  })
  .refine(
    data => { if (data.currentAvailable === true) return !!data.currentAvailableOn && data.currentAvailableOn.length > 0; return true },
    { message: 'Please select an availability date', path: ['currentAvailableOn'] }
  )
  .refine(
    data => { if (data.currentAvailable === true && data.currentAvailableOn) return new Date(data.currentAvailableOn) >= new Date(new Date().toDateString()); return true },
...
const vacancySchema = z.object({
  ...
  if (!data.vacancyApartmentType || data.vacancyApartmentType === 'No Option') {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vacancyApartmentType'], message: 'Select an apartment type' })
  }
  if (!data.vacancyState || !(ALLOWED_SWAP_STATES as readonly string[]).includes(data.vacancyState)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vacancyState'], message: 'Select a state' })
    return
  }
  ...
  if (!data.vacancyCity || !vacancyCities.includes(data.vacancyCity)) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vacancyCity'], message: 'Select a valid city' })
  }
  const vacancyAreas = data.vacancyCity ? getSwapAreasForCity(data.vacancyState as Location, data.vacancyCity) : []
  if (vacancyAreas.length > 0) {
    if (!data.vacancyArea) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vacancyArea'], message: 'Select an area' })
    } else if (!vacancyAreas.includes(data.vacancyArea)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vacancyArea'], message: 'Select a valid area' })
    }
  }
  if (!data.vacancyFeatures || data.vacancyFeatures.length === 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['vacancyFeatures'], message: 'Please select at least one feature' })
  }
})
...
const Engine: React.FC = () => {
  ...
  const toggleVacancyFeature = (f: string) =>
    setVacancyFeatures(prev => prev.includes(f) ? prev.filter(x => x !== f) : [...prev, f])
  ...
  const goNext = async () => {
    if (step === 0) {
      ...
      const result = step2Schema.safeParse({
        currentType, currentState, currentCity, currentArea, currentAvailable, currentRent,
        currentAvailableOn: currentAvailableOn || undefined, features: selectedFeatures,
      })
      if (!result.success) {
        const errs: AllErrors = {}
        result.error.issues.forEach(i => { errs[i.path[0] as keyof AllErrors] = i.message })
      ...
      const result = vacancySchema.safeParse({ hasVacancyAlert, vacancyApartmentType, vacancyState, vacancyCity, vacancyArea, vacancyFeatures })
      if (!result.success) {
        const errs: AllErrors = {}
        result.error.issues.forEach(i => { errs[i.path[0] as keyof AllErrors] = i.message })
        setErrors(errs)
  ...
  const handleCancel = () => {
    if (fromDashboard) { router.push('/dashboard'); return }
    router.back()
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await Client.post('/listings', payload, { Authorization: `Bearer ${token}` })

      if (response.status === 401) { unsetToken(); router.push('/login'); return }
      if (response.status === 403) { setAlertMsg('You do not have permission to create a listing.'); return }
      if (response.status === 422) { setAlertMsg('Some fields are invalid. Please review your details.'); return }
      if (response.status === 429) { setAlertMsg('Too many requests. Please wait a moment and try again.'); return }
      ...
      <GuestLayout>
        <div className="max-w-2xl mx-auto py-20 px-4 text-center">
          ...
          >
            ...
            <h2 className="text-3xl font-poppins-bold text-slate-900 mb-3">Application Submitted</h2>
            <p className="text-slate-500 font-poppins-regular mb-2">
              Your document is under review. We typically review applications within 24 hours.
            </p>
            <p className="text-slate-500 font-poppins-regular mb-8">
              You will be notified by <span className="font-poppins-bold text-slate-700">SMS and email</span> once your listing is approved.
            </p>
            <button
              type="button"
  ...
  if (listingMode === 'undecided' || (listingMode === 'SEEKING' && !seekerCategory)) {
    return (
      <GuestLayout>
        <div className="max-w-2xl mx-auto py-12 px-4">
          ...
          >
            ...
            <p className="text-slate-500 font-poppins-regular">First, let us know your current situation.</p>
          </motion.div>
          ...
          >
            <h3 className="text-xl font-poppins-bold text-slate-800 mb-2">Do you have an apartment to swap?</h3>
            <p className="text-sm text-slate-400 font-poppins-regular mb-6">This helps us find the right match for you.</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
              ...
              >
                ...
                <p className="font-poppins-bold text-slate-800 text-base">Yes, I have an apartment</p>
                <p className="text-xs text-slate-500 font-poppins-regular mt-1">I want to swap my current apartment for one in another location</p>
              </button>
              ...
              >
                ...
                <p className="font-poppins-bold text-slate-800 text-base">No, I'm looking for one</p>
                <p className="text-xs text-slate-500 font-poppins-regular mt-1">I need to find housing in a specific city</p>
              </button>
            </div>
            ...
            {listingMode === 'SEEKING' && (
              ...
              >
                <p className="text-sm font-poppins-bold text-slate-700 mb-3">Why are you looking? <span className="text-slate-400 font-poppins-regular">(Select one)</span></p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                  {SEEKER_CATEGORIES.map(cat => (
                    <button
                      ...
                      className={`flex items-start gap-3 rounded-xl border p-4 text-left transition-all ${seekerCategory === cat.value
                        ? 'border-emerald-500 bg-emerald-50'
                        : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'
                      }`}
                    >
                      <span className={`mt-0.5 ${seekerCategory === cat.value ? 'text-emerald-600' : 'text-slate-400'}`}>{cat.icon}</span>
                      <span>
                        <span className="block text-sm font-poppins-bold text-slate-800">{cat.label}</span>
                        <span className="block text-xs font-poppins-regular text-slate-500 mt-0.5">{cat.description}</span>
                      </span>
                    </button>
                  ))}
                </div>
  ...
  return (
    <GuestLayout>
      ...
      <div className="max-w-2xl mx-auto py-12 px-4">
        ...
        <StepHeader current={step} steps={listingMode === 'SEEKING' ? seekerSteps : undefined} />
        ...
        >
          <AnimatePresence mode="wait" initial={false}>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select value={desiredState} onChange={e => { setDesiredState(e.target.value as Location); setDesiredCity('') }} className={fc('desiredState')}>
                  ...
                  <div>
                    ...
                    <select value={desiredCity} onChange={e => setDesiredCity(e.target.value)} disabled={desiredState === 'No Option'} className={fc('desiredCity')}>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <select value={currentState} onChange={e => { setCurrentState(e.target.value as Location); setCurrentCity(''); setCurrentArea('') }} className={fc('currentState')}>
                      <option value="No Option" disabled>Select state…</option>
                      {ALLOWED_SWAP_STATES.map(state => <option key={state} value={state}>{state}</option>)}
                    </select>
                  ...
                  <div>
                    ...
                    <select value={currentCity} onChange={e => { setCurrentCity(e.target.value); setCurrentArea('') }} disabled={currentState === 'No Option'} className={fc('currentCity')}>
                  ...
                  <div>
                    ...
                    <select value={currentArea} onChange={e => setCurrentArea(e.target.value)} disabled={!currentCity || currentAreas.length === 0} className={fc('currentArea')}>
                ...
                <div>
                  ...
                  <div className={`grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 rounded-2xl transition-all ${errors.features ? 'bg-red-50 border border-red-300' : ''}`}>
                    {FEATURES.map(feat => (
                      <label key={feat} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all text-sm ${selectedFeatures.includes(feat) ? 'bg-emerald-50 border-emerald-500 text-emerald-700 font-poppins-bold' : 'border-slate-200 text-slate-500 font-poppins-regular'}`}>
              ...
              >
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <button type="button" onClick={() => setHasVacancyAlert(true)} className={`rounded-2xl border p-5 text-left transition-all ${hasVacancyAlert === true ? 'border-emerald-500 bg-emerald-50 shadow-lg shadow-emerald-500/10' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                    ...
                    <button type="button" onClick={() => setHasVacancyAlert(false)} className={`rounded-2xl border p-5 text-left transition-all ${hasVacancyAlert === false ? 'border-slate-900 bg-slate-900 text-white shadow-lg shadow-slate-900/10' : 'border-slate-200 hover:border-slate-300 hover:bg-slate-50'}`}>
                  ...
                  <div className="space-y-6 rounded-3xl border border-emerald-100 bg-emerald-50/60 p-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      ...
                      <div>
                        ...
                        <select value={vacancyState} onChange={e => { setVacancyState(e.target.value as Location); setVacancyCity(''); setVacancyArea('') }} className={fc('vacancyState')}>
                          <option value="No Option" disabled>Select state…</option>
                          {ALLOWED_SWAP_STATES.map(state => <option key={state} value={state}>{state}</option>)}
                        </select>
                      ...
                      <div>
                        ...
                        <select value={vacancyCity} onChange={e => { setVacancyCity(e.target.value); setVacancyArea('') }} disabled={vacancyState === 'No Option'} className={fc('vacancyCity')}>
                      ...
                      <div>
                        ...
                        <select value={vacancyArea} onChange={e => setVacancyArea(e.target.value)} disabled={!vacancyCity || vacancyAreas.length === 0} className={fc('vacancyArea')}>
                          <option value="" disabled>{!vacancyCity ? 'Select city first…' : vacancyAreas.length === 0 ? 'No areas configured yet' : 'Select area…'}</option>
                    ...
                    <div>
                      <label className="block text-sm font-poppins-medium text-slate-600 mb-3">Vacancy Features <span className="text-slate-400">(Select what applies)</span></label>
                      <div className={`grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 rounded-2xl transition-all ${errors.vacancyFeatures ? 'bg-red-50 border border-red-300' : ''}`}>
                        {FEATURES.map(feat => (
                          <label key={feat} className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all text-sm ${vacancyFeatures.includes(feat) ? 'bg-emerald-50 border-emerald-500 text-emerald-700 font-poppins-bold' : 'border-slate-200 text-slate-500 font-poppins-regular'}`}>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedFeatures.map(f => (
                      <span key={f} className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-700 px-3 py-1.5 rounded-lg font-poppins-medium">{f}</span>
                    ))}
                  </div>
                </div>
                    ...
                    >
                      <input
                        ...
                        onChange={e => {
                          ...
                          if (file) setErrors(prev => ({ ...prev, verificationFile: undefined }))
                        }}
                      />
                      {verificationFile ? (
                        <div className="flex flex-col items-center gap-2">
                          <CheckCircle2 size={36} className="text-emerald-500" />
                          <p className="font-poppins-bold text-slate-800 text-sm">{verificationFile.name}</p>
                          <p className="text-xs text-slate-400 font-poppins-regular">Click to change file</p>
                        </div>
                      ) : (
                        <div className="flex flex-col items-center gap-2">
                          <Upload size={36} className="text-slate-300" />
                          <p className="font-poppins-bold text-slate-600 text-sm">Click to upload document</p>
                          <p className="text-xs text-slate-400 font-poppins-regular">PDF, JPEG, or PNG — max 10 MB</p>
                        </div>
                      )}
                    </div>
            ...
            {listingMode === 'SEEKING' && step === 2 && (
              ...
              >
                <div>
                  <h3 className="text-2xl font-poppins-bold text-slate-800 mb-1">Confirm Your Search</h3>
                  <p className="text-sm text-slate-400 font-poppins-regular">Review your housing request before submitting.</p>
                </div>
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-2xl p-5">
                  <p className="text-[10px] font-poppins-bold text-slate-500 uppercase tracking-widest mb-2">Reason for Seeking</p>
          ...
          <div className={`flex flex-wrap gap-3 mt-8 pt-6 border-t border-slate-100 ${step > 0 || fromDashboard ? 'justify-between' : 'justify-end'}`}>
            <div className="flex items-center gap-3">
              {fromDashboard && (
                <button type="button" onClick={handleCancel} disabled={loading} className="flex items-center gap-2 px-6 py-3 rounded-xl border border-slate-200 text-slate-600 font-poppins-medium cursor-pointer transition-all duration-300 ease-out hover:bg-slate-50 hover:-translate-y-0.5 hover:shadow-md active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:translate-y-0 disabled:hover:shadow-none">
                  <X size={18} /> Cancel
                </button>
              )}
              {step > 0 && (
            ...
            {step < maxStep ? (
              <button type="button" onClick={goNext} disabled={loading} className="flex items-center gap-2 bg-emerald-600 text-white px-8 py-3 rounded-xl font-poppins-bold cursor-pointer transition-all duration-300 ease-out shadow-lg shadow-emerald-600/20 hover:bg-emerald-700 hover:-translate-y-0.5 hover:shadow-xl hover:shadow-emerald-600/25 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed">
                {loading ? <><Loader2 size={18} className="animate-spin" /> Please wait…</> : <>Next <ChevronRight size={18} /></>}
              </button>
            ) : (
