...
import { findMatches } from '@/services/MatchingEngine'; // adjust path as needed
...
const Dashboard: React.FC = () => {
  ...
  // Merge live user into the full request pool so the engine can see them
  const allRequests: SwapRequest[] = useMemo(() => {
    if (!currentUser) return MOCK_REQUESTS;
    // Replace any existing entry for this user, or prepend if new
    const others = MOCK_REQUESTS.filter(r => r.id !== currentUser.id);
      ...
      <div className="max-w-4xl mx-auto py-20 px-4 text-center">
        <div className="bg-white p-12 rounded-3xl shadow-sm border border-slate-200">
          ...
          <p className="text-slate-500 font-poppins-regular mb-8 max-w-md mx-auto">
            We need to know what you're leaving and what you're looking for to run the chain matching algorithm.
          </p>
          <Link
            href="/engine"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
        <div>
          ...
          <p className="text-slate-500 font-poppins-regular mt-2">
            Algorithm detected{' '}
            <span className="text-emerald-600 font-poppins-bold">{chains.length}</span>{' '}
            potential match chains for you.
          </p>
        </div>
        <Link
          href="/engine"
      ...
      <div className="grid grid-cols-1 gap-12">
        {chains.length === 0 ? (
          <div className="bg-white p-20 rounded-3xl border border-dashed border-slate-300 text-center">
            ...
            <h3 className="text-2xl font-poppins-bold text-slate-400">No chains found yet</h3>
      ...
      <div className="mt-20 bg-slate-900 text-white p-12 rounded-[2.5rem] relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="text-slate-400 font-poppins-regular leading-relaxed mb-8">
            TenantSwap only facilitates the connection. Once you connect with others in your chain,
